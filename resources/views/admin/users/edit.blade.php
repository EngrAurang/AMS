    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $user->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('joining_date') ? 'is-invalid' : '' }}" type="text" name="joining_date" id="joining_date" value="{{ old('joining_date', $user->joining_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_leaves') ? 'is-invalid' : '' }}" type="number" name="total_leaves" id="total_leaves" value="{{ old('total_leaves', $user->total_leaves) }}" step="0.01">
                @if($errors->has('total_leaves'))
                    <div class="invalid-feedback">
                        {{ $errors->first('total_leaves') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.total_leaves_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="leaves_taken">{{ trans('cruds.user.fields.leaves_taken') }}</label>
                <input class="form-control {{ $errors->has('leaves_taken') ? 'is-invalid' : '' }}" type="number" name="leaves_taken" id="leaves_taken" value="{{ old('leaves_taken', $user->leaves_taken) }}" step="0.01">
                @if($errors->has('leaves_taken'))
                    <div class="invalid-feedback">
                        {{ $errors->first('leaves_taken') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.leaves_taken_helper') }}</span>
            </div>
            <div class="form-group">
